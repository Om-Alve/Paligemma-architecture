...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        embeddings = patch_embeds + positional_embeddings
...
class SiglipAttention(nn.Module):
    ...
    def forward(
        ...
        w_q = self.q_proj(hidden_state)
        w_k = self.k_proj(hidden_state)
        w_v = self.v_proj(hidden_state)

        w_q = w_q.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )
        w_k = w_k.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )
        w_v = w_v.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )

        attn_weights = torch.matmul(w_q, w_k.transpose(2, 3)) * self.scale
        ...
        attn_output = torch.matmul(attn_weights, w_v)
...
class SiglipVisionTransformer(nn.Module):
    def __init__(self, config: SiglipVisionConfig) -> None:
        ...
        self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_epsilon)
